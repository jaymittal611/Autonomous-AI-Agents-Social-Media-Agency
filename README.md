...
| **Community Agent** | Reads inbound comments from the platform simulation, drafting brand-safe replies. | `Mock Database` ➔ `CommunityAction` |
| **Analytics Agent** | Decodes mock platform telemetry to extract empirical signals and prescriptive changes for next cycle. | `Mock Database` ➔ `WeeklyAnalysisReport` |
...
Inter-agent communication is decoupled and fully observable via `agent_tracer.py` . Every message, state transition, and compliance rejection is persisted to (Available to download on UI):
